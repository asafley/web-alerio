<section id="contact" class="py-5">
    <div class="container">
        <div class="text-center" style="max-width: 800px; margin-left: auto; margin-right: auto;">
            <h2 class="display-5 fw-bold mb-4">Let's Build Something Amazing</h2>
            <p class="fs-5 text-muted mb-5">Ready to transform your vision into reality?</p>
            <a href="/contact" class="btn btn-lg btn-primary rounded-pill px-5 py-3 fw-semibold" style="background: linear-gradient(135deg, #4f46e5 0%, #a855f7 100%); border: none; box-shadow: 0 10px 30px rgba(79, 70, 229, 0.3);">
                Contact Us Today
            </a>
        </div>
    </div>
</section>
