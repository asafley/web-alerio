        ...
        <script>
            async function loadUsettings() {
                try {
                    ...
                    if (posts.length === 0) {
                        tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted"><div class="py-4"><p>No settings found</p><a href="/admin/settings/create" class="btn btn-sm btn-primary">Add Post</a></div></td></tr>';
                        return;
                    }
                    ...
                    posts.forEach(function(post, index, array) {
                        ...
                        row.innerHTML = `
                            ...
                            <td>
                                <a href="/admin/settings/edit/${post.id}" class="btn btn-sm btn-primary">Edit</a>
                                <button class="btn btn-sm btn-danger" onclick="deleteUsetting(${post.id})">Delete</button>
                                <form id="delete-${post.id}" action="/admin/settings/${post.id}" method="POST" style="display: none;">
                    ...
                    const tbody = document.querySelector('table tbody');
                    tbody.innerHTML = '<tr><td colspan="6" class="text-center text-danger">Error loading settings</td></tr>';
                }
            }

            function deleteUsetting(postId) {
            ...
            document.addEventListener('DOMContentLoaded', loadUsettings);
        </script>
    </div>
